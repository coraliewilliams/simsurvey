...
survey_raw <- read_excel("~/Projects/simsurvey/data/survey/simulationSurvey_extraction_100papers_21112023.xlsx", na=c("", "NA", NA))
...
supp1 <- create_plot(survey, r, "Reporting practices (all stages)")
...
supp2 <- create_plot(survey, rs, "Reporting section practices (all stages)")
...
# save 
ggsave("output/Figure3_ReportingPractices_allStages.png",Figure3)
...
mean(survey$`2.5.1.datagen_parameters`, na.rm=T)
min(survey$`2.5.1.datagen_parameters`, na.rm=T)
max(survey$`2.5.1.datagen_parameters`, na.rm=T)

mean(survey$`2.6.1.datagen_conditions`, na.rm=T)
min(survey$`2.6.1.datagen_conditions`, na.rm=T)
max(survey$`2.6.1.datagen_conditions`, na.rm=T)

mean(survey$`2.7.1.datagen_simulationruns`, na.rm=T)
min(survey$`2.7.1.datagen_simulationruns`, na.rm=T)
max(survey$`2.7.1.datagen_simulationruns`, na.rm=T)
...
# make barplot
barplot(estimand_counts,
        main="Estimand / Statistical target",
        xlab="",            
        ylab="Counts",             
        col=rainbow(length(estimand_counts), start=0, end=.3),
        border="darkgrey")
