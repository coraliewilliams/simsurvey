...
# Vector of distributions from your data
...
count_distributions <- function(dist_vector) {
  ...
  # Initialise counts for each distribution
...
# old code
# not sure how to treat "beta binomial" and total normal and total binomial
counts <- c(
    "Normal" = length(grep("normal", dist_vector, ignore.case = TRUE)) - length(grep("multivariate normal", dist_vector, ignore.case = TRUE)) - length(grep("truncated normal", dist_vector, ignore.case = TRUE)),
...
# set up color palette with 12 colors
...
ggplot(df_distribution_counts, aes(x = Distribution, y = Counts, fill = Distribution)) +
  ...
  theme_minimal() +
  labs(title = "Statistical Distributions", x = "Distribution", y = "Counts") +
