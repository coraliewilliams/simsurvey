...
In this study we surveyed 100 research articles in ecology and evolution journals that included simulation studies evaluating statistical methods. The following HTML webpage provides the code necessary to reproduce the analysis conducted in the paper, along with detailed explanations of the methods and results.
...
::: column-margin
Note, exclusion term criteria were refined post-hoc.
:::
...
# Plot histogram
plot_dist <- ggplot(dist_summary, aes(x = Distribution, y = Percentage, fill = Distribution)) +
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Set3") +
  theme_minimal() +
  scale_y_continuous(breaks = seq(0,50, by=5)) +
  labs(title = "Statistical Distributions", x = "", y = "percentage of occurences") +
...
# get percentage from counts
estimand_perct <- estimand_counts %>% 
  mutate(Freq = Freq / sum(Freq) * 100)

# Plot histogram
plot_estarget <- ggplot(estimand_perct, aes(x = reorder(estimand_list, -Freq), y = Freq, fill = estimand_list)) +
  geom_bar(stat = "identity") +
  scale_fill_brewer(palette = "Set3") +
  theme_minimal() +
  #coord_flip() +
  labs(title = "Statistical target", x = "", y = "percentage of occurences") +
...
# Plot histogram with ordered categories
plot_perf <- ggplot(perf_dat, aes(x = reorder(category, -percentage), y = percentage, fill = category)) +
  geom_bar(stat = "identity") +
  #fill=c(rep("skyblue",8),"#d9700f","skyblue")
  theme_minimal() +
  scale_fill_brewer(palette = "Set3") +
  labs(title = "Performance Measure", x = "", y = "percentage of occurences") +
  #coord_flip() +
...
# make into factor with same order as in excel sheet
datcrosscheck$survey_question <- factor(datcrosscheck$survey_question,
                                        levels = rev(datcrosscheck$survey_question))

# make lollipop plot of crosscheck agreement
p <- ggplot(datcrosscheck, aes(x = crosscheck_agreement_percent, y = survey_question)) +
  geom_segment(aes(x = 0,
                   xend = crosscheck_agreement_percent,
                   yend = survey_question), colour = "darkgray") +
