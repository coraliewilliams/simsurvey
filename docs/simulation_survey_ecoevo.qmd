...
survey_raw <- read_excel("~/Projects/simsurvey/data/survey/simulationSurvey_extraction_100papers_18012024.xlsx", na=c("", "NA", NA))
...
# histogram 
ggplot(datgen_pars_clean, aes(x=pars)) +
  ...
  theme(axis.text.x = element_text(size=14)) +
  theme(axis.text.y = element_text(size=14)) +
  labs(title = "Data-generating mechanism: number of parameters", x = "No. parameters", y = "frequency")
...
# IMPROVE: rename level "NA" into "not stated"
#datgen_runs[is.na(datgen_runs)] <- "not stated"



# histogram 
ggplot(datgen_runs, aes(x=runs)) +
  ...
  theme(axis.text.x = element_text(size=14)) +
  theme(axis.text.y = element_text(size=14)) +
  labs(title = "Data-generating mechanism: number of simulation runs", x = "No. runs", y = "frequency")
...
#table(survey$`2.7.3.datagen_simulationruns_justification`)
