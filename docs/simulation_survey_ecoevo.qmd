...
# Function to categorise distribution types
categorise_distribution <- function(dist_string) {
  categories <- case_when(
    grepl("normal", dist_string, ignore.case = TRUE) & !grepl("multivariate", dist_string, ignore.case = TRUE) ~ "Normal",
    grepl("multivariate normal", dist_string, ignore.case = TRUE) ~ "Multivariate normal",
    grepl("beta", dist_string, ignore.case = TRUE) ~ "Beta",
    grepl("gamma", dist_string, ignore.case = TRUE) ~ "Gamma",
    grepl("Poisson", dist_string, ignore.case = TRUE) ~ "Poisson",
    grepl("binomial|bernoulli", dist_string, ignore.case = TRUE) ~ "Binomial",
    grepl("dirichlet", dist_string, ignore.case = TRUE) ~ "Dirichlet",
    grepl("uniform", dist_string, ignore.case = TRUE) ~ "Uniform",
    grepl("exponential", dist_string, ignore.case = TRUE) ~ "Exponential"
    #TRUE ~ "Other"
  )
  return(categories)
}
...
new <- survey %>%
  # Separate each distribution type into a new row
  separate_rows(`2.8.datagen_distribution`, sep = " and |, ") %>%
  # Categorise each distribution type
  mutate(category = categorise_distribution(`2.8.datagen_distribution`)) %>%
  # Keep only relevant columns
  select(paper_title, DOI, category)

table(new$category)

# Create the histogram plot
ggplot(new, aes(x = category, fill = category)) +
  geom_bar() +
  scale_fill_brewer(palette = "Set2") +  # Color-blind friendly palette
  theme_minimal() +
  labs(
    title = "Distribution of Data Generation Categories",
    x = "Category",
    y = "Frequency",
    fill = "Category"
  )
...
###### 9: Case study/working example



###### 10: Analysis performance measures




###### 11: Monte Carlo error
```
